...
- **The pit-decision classifier is not usable as a decision rule yet.** It scores test ROC-AUC 0.98, but pit events are only 4.8% of laps (48 of 995) — and at that prevalence ROC-AUC flatters badly. Test PR-AUC is 0.25 and F1 is 0.077, i.e. precision ≈ 0.04 at the default 0.5 threshold. The model *ranks* laps well; its operating point is not tuned. No class weighting, resampling or threshold search is applied yet. The selected lap-time regressor likewise has test R² −0.17 (it is chosen on cross-validated MAE, where it is positive) — an honest extrapolation failure on the final stint of a single race, not a bug. Full per-fold tables: [`artifacts/reports/model_selection_report.md`](artifacts/reports/model_selection_report.md).
