...
⚠️ **Read the classifier row carefully.** Pit events are 4.8% of laps (48 of 995). ROC-AUC 0.98 says the
model *ranks* laps well; PR-AUC 0.25 and F1 0.077 say that at the default 0.5 threshold it is not usable
as a decision rule — precision is ~0.04. No class weighting, resampling or threshold tuning is applied
yet. The ranking is real; the operating point is not. Reporting ROC-AUC alone here would be exactly the
mistake this section's own guidance warns against.
