...
Metric computation for both Task 7 problems.

Copied verbatim from Task 6's ``app.intelligence.ml.evaluation`` so the DL
and classical numbers in the comparison table are computed by the *same
code*, not merely by the same-named metric. Every function here is a pure
...
which can happen when a validation block contains no pit event at all) are reported as ``None`` with a reason
...
def classification_metrics(y_true, y_pred, y_proba=None) -> dict:
    ...
    if n_classes_true < 2:
        undefined_reason = (
            ...
            "no pit event falls inside this split; "
