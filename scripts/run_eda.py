...
def main(data_dir: Path, output_dir: Path, regenerate: bool) -> int:
    ...
    # --- 8. Publish the canonical Task 5 input contract ------------------
    # `fastf1_laps_clean.csv` is the shared hand-off to Feature Engineering
    # (Task 5) and, downstream, Machine Learning (Task 6). Publish it to the
    # shared data/processed location rather than leaving it only inside this
    # run's artifact dump, so every module reads one canonical copy.
    DATA_PROCESSED_DIR.mkdir(parents=True, exist_ok=True)
    cleaned["fastf1_laps"].to_csv(FASTF1_LAPS_CLEAN_CSV, index=False)
    log.info("Published canonical clean dataset -> %s", FASTF1_LAPS_CLEAN_CSV)

    # Propagate real-vs-synthetic provenance so Task 5/6 and the frontend
    # never have to guess or assume.
    if RAW_DATA_SOURCE_MARKER.exists():
        PROCESSED_DATA_SOURCE_JSON.write_text(RAW_DATA_SOURCE_MARKER.read_text())
    else:
        PROCESSED_DATA_SOURCE_JSON.write_text(json.dumps({"source": "synthetic"}, indent=2))
    log.info("Wrote data-source provenance -> %s", PROCESSED_DATA_SOURCE_JSON)
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output-dir", type=Path, default=DATA_ENGINEERING_ARTIFACTS_DIR)
