...
deployed race-strategy model will ever be in.

Task 7 imports ``app.intelligence.ml.splits`` and ``.evaluation`` directly
rather than copying them, so the deep-versus-classical comparison is computed
by literally the same code as Task 6's own numbers. The grid is deliberately small
